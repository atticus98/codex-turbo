...
> **核心原则**：最大化并行、最小化阻塞 —— 将任务拆解为 **可独立执行且互不冲突** 的子任务，通过 `multi_agent`（CLI 界面常显示为 Multi-agents）并行调度，待全部结果返回后整合为阶段性产出，再递归拆解下一轮，直至任务完成。
...
### 2. 并行调度
- 将所有 **无前置依赖** 的子任务打包，通过 `multi_agent` 同时下发
...
- **测试驱动**：可测试设计，单元测试覆盖，后台执行单元测试时，建议设置最大超时时间 60s（可按项目覆盖），避免任务卡死。
- **质量保证**：静态检查、格式化、代码审查
- **持续验证**：自动化测试和集成验证
...
> ❌ **反模式**：默认场景下滥用超长绝对路径。最佳实践是优先短路径或文件名；在调试、评审、定位问题时，使用 `file:line` 提供可追溯定位信息。
